...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <form method="GET" action="{{ route('actividades.index') }}" class="mb-6">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('actividades.edit', $item->id) }}"
                                ...
                                <form method="POST" action="{{ route('actividades.deactivate', $item->id) }}" class="inline-block"
                                      onsubmit="return confirm('¿Anular la actividad {{ $item->nombre }}?');">
                                    @csrf
                                    <button type="submit"
    ...
    <div class="mt-4 flex items-center justify-between text-sm text-gray-500">
        <span>
            Mostrando {{ $items->firstItem() ?? 0 }}–{{ $items->lastItem() ?? 0 }} de {{ $items->total() }} actividades
        </span>
        <div>
