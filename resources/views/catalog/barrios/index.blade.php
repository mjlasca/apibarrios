...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <form method="GET" action="{{ route('barrios.index') }}" class="mb-6">
        <div class="flex items-end gap-3">
            <div class="flex-1 max-w-md">
                <label for="q" class="block text-sm font-medium text-gray-700 mb-1">Buscar</label>
                <input type="text" id="q" name="q" value="{{ $keyword }}" placeholder="ID o nombre del barrio..."
                    autocomplete="off"
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Suma Muerte</th>
                ...
                @forelse ($items as $item)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">
                            @if ($item->suma_muerte)
                                ${{ number_format($item->suma_muerte, 0, ',', '.') }}
                            @else
                                —
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('barrios.edit', $item->reg) }}"
                                ...
                                <form method="POST" action="{{ route('barrios.deactivate', $item->reg) }}" class="inline-block"
                                      onsubmit="return confirm('¿Anular el barrio {{ $item->nombre }}?');">
                                    @csrf
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400">
                            Sin barrios para los filtros seleccionados
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="mt-4 flex items-center justify-between text-sm text-gray-500">
        <span>
            Mostrando {{ $items->firstItem() ?? 0 }}–{{ $items->lastItem() ?? 0 }} de {{ $items->total() }} barrios
        </span>
        <div>
