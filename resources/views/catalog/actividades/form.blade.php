@extends('layouts.app')
@section('title', $item ? 'Editar Actividad' : 'Nueva Actividad')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <header class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ $item ? 'Editar Actividad' : 'Nueva Actividad' }}</h1>
    </header>
    ...
    <form method="POST" action="{{ $item ? route('actividades.update', $item->id) : route('actividades.store') }}" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        @if ($item)
            @method('PUT')
        @endif
